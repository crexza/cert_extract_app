...
def process_single_page_task(pg_file, pg_num, page_text, is_service, valid_types):
    ...
    try:
        ...
        Extract the target fields carefully. If dates are written in words (e.g. 'January 13, 2026'), convert them to ISO format 'YYYY-MM-DD'.
        If a target field is missing from the document, set its value to an empty string ("").
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "- 'lot': string cylinder/batch tracking lot number. CRITICAL RULE: If no batch tracking lot number or cylinder lot number is explicitly mentioned, look for a 'Report No' or 'Report Number' in the document text and place that value here. Do not confuse this with the Certificate reference number (`cert`). If both are missing, set to '')\n"
                        "- 'type': string matching one of the provided target collections based on asset keyword contexts.\n\n"
                        "CRITICAL CLASSIFICATION RULE:\n"
                        "1. If the document text or model description contains 'rigrat', 'mx4', or 'detective+', you MUST force the 'type' field calculation value to be exactly: 'AREA MONITOR'.\n"
                        "2. If the document text mentions gas detection, calibration gases, gas cylinders, or displays metrics for combustible/toxic sensors (like H2S, O2, CO, LEL) but does NOT explicitly match the 'AREA MONITOR' keywords above, you MUST classify its type as: 'GD'."
                    )
                },
        ...
        item = json.loads(inference.choices[0].message.content)
        ...
        if not item.get("serial"): item["serial"] = "MANUAL_ENTRY_REQUIRED"
        if not item.get("model"):  item["model"] = "Unknown Model"
        if not item.get("cert"):   item["cert"] = "PENDING"
        if not item.get("lot"):    item["lot"] = ""
        
        if not item.get("calibration_date"): item["calibration_date"] = today_str
        if not item.get("expiry_date"):      item["expiry_date"] = today_str
        ...
        if item["serial"] == "MANUAL_ENTRY_REQUIRED" and item["model"] == "Unknown Model":
            item["type"] = "UNRESOLVED"
            item["target_collection"] = "UNRESOLVED"
            item["is_valid"] = False
            item["error_explanation"] = "Asset metadata could not be fully resolved by AI."
        else:
            item["type"] = detected_type
            item["target_collection"] = detected_type + ("_SERVICE" if is_service else "")
            item["is_valid"] = True
            item["error_explanation"] = f"Successfully parsed via AI mapping into collection: {detected_type}."

        return item

    except Exception as error:
        logger.error(f"Mapping pipeline failure on page {pg_num}: {str(error)}")
        return {
            "id": f"STAGE_ERR_{uuid.uuid4().hex[:6].upper()}",
            "serial": "MANUAL_ENTRY_REQUIRED",
            "model": "Unknown Model",
